...
static PF_Err ParamsSetup(PF_InData* in_data, PF_OutData* out_data)
{
    ...
    // 100 = the digital scale tracks the lens zoom (less crop at the wide end, follows the zoom);
    // 0 = one constant scale for the whole clip (steady, but the wide end is cropped to match the
    // long end). On a fixed-focal lens both ends are identical, so this does nothing.
    ...
    PF_LayerDef* output)
{
    ...
    if (inst && inst->gyroIntegrator) {
        ...
        sp.zoomSteadiness     = 1.0f - scaleFollows / 100.0f;   // 100% follows → steadiness 0 (adaptive)
