...
struct StabilizationParams {
    ...
    float zoomEaseScale      = 1.0f;    // extra digital zoom (>=1) that makes the APPARENT zoom
                                        // follow a smoothed focal, easing the lens's hard start/
                                        // stop. Clip-normalized (widest-needed frame = 1) so it
                                        // never exposes new border. 1 = off ("Scale Follows Zoom" 0).
